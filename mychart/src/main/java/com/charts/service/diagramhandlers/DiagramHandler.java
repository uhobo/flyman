...
public abstract class DiagramHandler {
	...
	private final Logger log = LoggerFactory.getLogger(DiagramHandler.class);
	...
	protected void setDiagramData(ChartRequest chartRequest, ChartResponse chartResponse) throws Exception{
    	...
    	//diagramChart.addTitle(chartResponse.getOptions(), chartTitle);
    	chartResponse.setData(diagramChart);
    }
	...
	protected void buildDistributionDiagram(DiagramChart diagramChart) {
		...
		Integer itrIndex = 1;
		diagramChart.setStack(true);

		for(SeriesItem seriesItem : fileData.getSeriesList()) {
			diagramChart.getLabels().add(ChartAppUtil.getSeriesLabel(seriesItem));
		}
		...
		for(TreeNode treeNode: selectedNodeList){
    		...
    		diagramDataset.setBackgroundColor(color);
    		diagramDataset.setBorderColor(color);
			diagramDataset.setLabel(treeNode.getLabel());
			diagramDataset.setStack(treeNode.getLabel());
			for(SeriesItem seriesItem : fileData.getSeriesList()) {
				Double totalCount = getCountForSeries(getSelectedItems(treeNode), seriesItem);	
	...
	protected double calculatePercentage(double obtained, double total) {
	        return ChartAppUtil.round(obtained * 100 / total, 2) ;
	 }
	...
	protected String getSelectedCategoriesTitle(List<Integer> headerIdList) {
		final StringBuilder builder = new StringBuilder();
		IntStream.range(0, headerIdList.size()).forEach(x -> {
			int index = headerIdList.get(x);
	...
	protected String getPopulationTitle(List<Integer> dataLineIndexList) {
		final StringBuilder builder = new StringBuilder();
		Map<String, Integer> groupData = new HashMap<>();
		
		
		dataLineIndexList.stream().forEach(x -> {
			Integer total = groupData.get("total");
			if(total == null) {
				total = 0;
			}
			groupData.put("total", total++);
			if(this.fileData.getGroupColIndex() != null) {
				String groupName = this.fileData.getDataLines().get(x).getData().get(this.fileData.getGroupColIndex()).toString();
